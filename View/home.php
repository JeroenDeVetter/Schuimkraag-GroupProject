<?php?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <meta name="keywords" content="Webdevelopment, DeSchuimkraag, Bier, E-shop">
    <meta name="description" content="Home pagina">
    <meta name="author" content="Danny, David, Geert, Kristel Jeroen,">
    <link href="https://fonts.googleapis.com/css?family=Rubik&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="../public/css/navbar.css">
    <link rel="stylesheet" href="../public/css/contact.css">
    <link rel="stylesheet" href="../public/css/home.css">
    <link rel="stylesheet" href="../public/css/footer.css">
    <link rel="stylesheet" href="../public/css/cookies.css">
    <title>home</title>
</head>

<body> 

   <?php require "nav.php"?>
   
    <div class="container" id="container">
        <div class="row">
            <div class="heading-title text-center">
            </div>
            <h1> HET SCHUIMKRAAG-TEAM</h1>
            <div class="col-md-4 col-sm-4">
                <div class="team-member">
                    <div class="team-img">
                        <img src="../public/images/kristel.png" alt="team member" class="img-responsive">
                    </div>
                    <div class="team-hover">
                        <div class="desk">

                            <p><q>Tried running. Gave it up. Kept spilling my beer</q></p>
                        </div>
                        <div class="s-link">
                            <a href="underconst.php"><i class="fa fa-facebook"></i></a>
                            <a href="underconst.php"><i class="fa fa-twitter"></i></a>
                            <a href="underconst.php"><i class="fa fa-google-plus"></i></a>
                        </div>
                    </div>
                </div>
                <div class="team-title">
                    <h5>Kristel</h5>
                    <span>Front-end &amp; Lay-out</span>
                </div>
            </div>
            <div class="col-md-4 col-sm-4">
                <div class="team-member">
                    <div class="team-img">
                        <img src="../public/images/danny.png" alt="team member" class="img-responsive">
                    </div>
                    <div class="team-hover">
                        <div class="desk">

                            <p><q>I'm not really a social drinker. I'd say most of my drinking is work related</q></p>
                        </div>
                        <div class="s-link">
                            <a href="underconst.php"><i class="fa fa-facebook"></i></a>
                            <a href="underconst.php"><i class="fa fa-twitter"></i></a>
                            <a href="underconst.php"><i class="fa fa-google-plus"></i></a>
                        </div>
                    </div>
                </div>
                <div class="team-title">
                    <h5>Danny</h5>
<<<<<<< HEAD
                    <span>Database &amp; Back-end</span>
=======
                    <span>Database &amp Back-end </span>
>>>>>>> 8b19e054d4a639c52d91c877cf833758812c9379
                </div>
            </div>
            <div class="col-md-4 col-sm-4">
                <div class="team-member">
                    <div class="team-img">
                        <img src="../public/images/geert.png" alt="team member" class="img-responsive">
                    </div>
                    <div class="team-hover">
                        <div class="desk">

                            <p><q>With beers like this, who needs memories ...</q></p>
                        </div>
                        <div class="s-link">
                            <a href="underconst.php"><i class="fa fa-facebook"></i></a>
                            <a href="underconst.php"><i class="fa fa-twitter"></i></a>
                            <a href="underconst.php"><i class="fa fa-google-plus"></i></a>
                        </div>
                    </div>
                </div>
                <div class="team-title">
                    <h5>Geert</h5>
                    <span>Back-end &amp; PHP-Wizz</span>
                </div>
            </div>
            <div class="col-md-4 col-sm-4">
                <div class="team-member">
                    <div class="team-img">
                        <img src="../public/images/jeroen.png" alt="team member" class="img-responsive">


                    </div>
                    <div class="team-hover">
                        <div class="desk">

                            <p><q>Beer : Technically a solution</q></p>
                        </div>
                        <div class="s-link">
                            <a href="underconst.php"><i class="fa fa-facebook"></i></a>
                            <a href="underconst.php"><i class="fa fa-twitter"></i></a>
                            <a href="underconst.php"><i class="fa fa-google-plus"></i></a>
                        </div>
                    </div>
                </div>
                <div class="team-title">
                    <h5>Jeroen</h5>
                    <span>Back-end &amp; GIT-Master</span>
                </div>
            </div>
            <div class="col-md-4 col-sm-4">
                <div class="team-member">
                    <div class="team-img">
                        <img src="../public/images/david.png" alt="team member" class="img-responsive">
                    </div>
                    <div class="team-hover">
                        <div class="desk">

                            <p><q>You know things will get funky when i say: "hold my beer and watch this".</q></p>
                        </div>
                        <div class="s-link">
                            <a href="underconst.php"><i class="fa fa-facebook"></i></a>
                            <a href="underconst.php"><i class="fa fa-twitter"></i></a>
                            <a href="underconst.php"><i class="fa fa-google-plus"></i></a>
                        </div>
                    </div>
                </div>
                <div class="team-title">
                    <h5>David</h5>
                    <span>Front-end &amp; Loopholes</span>
                </div>
            </div>
            <div class="col-md-4 col-sm-4">
                <div class="team-member">
                    <div class="team-img">
                        <img src="../public/images/vraagteken.png" alt="team member" class="img-responsive">

                    </div>
                    <div class="team-hover">
                        <div class="desk">

                            <p><q>I'd love to work @ de Schuimkraag !</q></p>
                        </div>
                        <div class="s-link">
                            <a href="underconst.php"><i class="fa fa-facebook"></i></a>
                            <a href="underconst.php"><i class="fa fa-twitter"></i></a>
                            <a href="underconst.php"><i class="fa fa-google-plus"></i></a>
                        </div>
                    </div>
                </div>
                <div class="team-title">
                    <h5>????????</h5>
                    <span>Your passion</span>
                </div>
            </div>

        </div>
        <div class="overlay">
            <div class="popup">
                <h2>Hallo, deze site maakt gebruik van cookies!</h2>
                <h1><span class="higher">&#x1F36A;</span><span class="lower">&#x1F36A;</span></h1>
                <p>We gebuiken session en functionele cookies om uw bezoek aan de website zo aangenaam mogelijk te maken.</p>
                <div class="btn-container">
                    <button class="btn" id="btnOK">Ik begrijp het</button>
                    <a href='underconst.php'><button class="btn">Meer info</button></a>
                    <button class="btn ghost" id="trick">
                        Nee, Ik ben op dieet !
                        <span>Niet op mij klikken &#128540;</span>
                    </button>
                </div>
            </div>
        </div>
    </div>
    <?php require "footer.php"?>
    <script src="//code.jquery.com/jquery-1.11.3.min.js "></script>
    <!-- Latest compiled and minified JavaScript -->
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.min.js "></script>
   <script src="../public/js/cookies.js"></script>
    <script src="../public/js/home.js"></script>
    <script src="../public/js/footer.js "></script>





</body>

</html>